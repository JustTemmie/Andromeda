...
class Language(commands.Cog):
    ...
    async def language_text_command(self, ctx: commands.Context, *, language: str = None):
        if language is None:
            current_language_ID = DbLib.language_database.read_value(ctx.author.id)
        ...
        await self.bot.lang.tr_send(ctx, "language_setter_success", userID=ctx.author.id, new_language=new_language)
    ...
    async def language_slash_command(
        ...
        if language is None:
            current_language_ID = DbLib.language_database.read_value(intercation.user.id)
            if current_language_ID:
                current_language = self.get_language_friendly_name(current_language_ID)
        ...
        new_language = f"{language} ({self.get_language_friendly_name(language)})"
        await intercation.response.send_message(self.bot.lang.tr("language_setter_success", intercation=intercation, new_language=new_language))
    
    def set_database_value(self, userID, new_language) -> str:
        if DbLib.language_database.read_value(userID):
            DbLib.language_database.update(userID, "preferred_language", new_language)

        else:
            DbLib.language_database.write(userID, new_language)
